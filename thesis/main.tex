...
self.actor_train_op = tf.train.AdamOptimizer(LR).minimize(self.actor_loss)
	...
	\subsection{训练结果}
	\subsubsection{两种目标函数效果对比}
	上文\ref{ppocontent}节提到了PPO算法有两种可选的目标函数，一种是带惩罚项的动态$\lambda$调整目标函数，另一种是使用裁剪概率比率限制策略更新幅度的目标函数。
	...
	\begin{figure}[h]
		...
		\label{fixed-point}
	\end{figure}
	\subsubsection{探究归一化对训练结果的影响}
	\subsubsection{启用多线程加速训练过程}
	\subsection{总结}
