...
\begin{document}
	...
	\begin{algorithm}[H]
		...
		\For{$i\in{1, \dots, N}$}{
			...
			\For{$j\in{1, \dots, M}$}{
				$J_{PPO}(\theta)=-\sum_{t=1}^{T}(\hat{A_t})^2$\\
				根据目标函数$J_{PPO}(\theta)$更新$\theta$
			}
			\For{$j\in{1, \dots, B}$}{
				$L_{BL}(\phi)=\sum_{t=1}^{T}\frac{\pi_\theta(a_t|s_t)}{\pi_{old}(a_t|s_t)}\hat{A_t}-\lambda KL[\pi_{old}|\pi_\theta]$\\
...
self.actor_train_op = tf.train.AdamOptimizer(LR).minimize(self.actor_loss)
	...
	\begin{figure}[h]
		\centering
		\subfigure[训练过程中的reward变化]{
			\includegraphics[width=7cm]{./pic/unity-reward.pdf}
		}
		\subfigure[训练过程中的loss变化]{
			\includegraphics[width=7cm]{./pic/unity-loss.pdf}
		}
